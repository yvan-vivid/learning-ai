...
class TrainingSequencer(Generic[IV, OV]):
    ...
    def training_set(self, words: Iterable[str]) -> TrainingSet:
        ...
        for word in words:
            for x, y in self.generator(word):
                xs.append(self.in_encoder.encode(x))
                ys.append(self.out_encoder.encode(y))
        return tensor(xs), tensor(ys)

    def freq_sequence(self, words: Iterable[str]) -> FreqTrainingSet:
        for word in words:
            for x, y in self.generator(word):
